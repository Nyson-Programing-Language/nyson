                                ...
                                if skips == 0 {
                                    if skip == false {
                                        if (vec[y] == "\"" || vec[y] == "\'" || vec[y] == r"\`") && vec[y-1] != "\\" {
                                            ...
                                            string.push_str(key().to_string().as_str());
                                            let mut leng = 0;
                                            let mut n2 = 0;
                                            let mut skip1 = false;
                                            for f in y+1..vec.len() {
        ...
        if skips == 0 {
            if skip == false {
                if (vec[y] == "\"" || vec[y] == "\'" || vec[y] == r"\`") && vec[y-1] != "\\" {
                    ...
                    file_s.push_str(key().to_string().as_str());
                    let mut leng = 0;
                    let mut n2 = 0;
                    let mut skip1 = false;
                    for f in y+1..vec.len() {
        ...
        if skips == 0 {
            if skip == false {
                if (text[y] == "\"" || text[y] == "\'" || text[y] == r"\`") && text[y-1] != "\\" {
                    ...
                    text_s.push_str(key().to_string().as_str());
                    let mut leng = 0;
                    let mut n2 = 0;
                    let mut skip1 = false;
                    for f in y+1..vec.len() {
        ...
        if skips == 0 {
            if skip == false {
                if vec[y] == "\"" || vec[y] == "\'" || vec[y] == r"\`" {
                    ...
                    imput_s.push_str(key().to_string().as_str());
                    let mut leng = 0;
                    let mut n2 = 0;
                    let mut skip1 = false;
                    for f in y+1..vec.len() {
        ...
        if skips == 0 {
            if skip == false {
                if find[y] == "\"" || find[y] == "\'" || find[y] == r"\`" {
                    ...
                    find_s.push_str(key().to_string().as_str());
                    let mut leng = 0;
                    let mut n2 = 0;
                    let mut skip1 = false;
                    for f in y+1..vec.len() {
...
fn key() -> String {
    ...
    for c in stdin.keys() {
        match c.unwrap() {
            Key::Char('q') => break,
            Key::Char(c) => key = c.to_string().as_str().parse().unwrap(),
            Key::Alt(c) => {
                key = "^".parse().unwrap();
                key.push_str(c.to_string().as_str());
            },
            Key::Ctrl(c) => {
                key = "*".parse().unwrap();
                key.push_str(c.to_string().as_str());
            },
            Key::Esc => key = "ESC".parse().unwrap(),
            Key::Left => key = "←".parse().unwrap(),
            Key::Right => key = "→".parse().unwrap(),
            Key::Up => key = "↑".parse().unwrap(),
            Key::Down => key = "↓".parse().unwrap(),
            Key::Backspace => key = "×".parse().unwrap(),
            _ => {}
        }
        break;
    }
